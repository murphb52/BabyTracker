...
public struct TimelineDayGridView: View {
                ...
                ZStack(alignment: .topLeading) {
                    ...
                    if Calendar.autoupdatingCurrent.isDateInToday(day) {
                        TimelineView(.everyMinute) { context in
                            currentTimeLine(at: context.date, columnWidth: columnWidth, columnCount: grid.columns.count)
                        }
                    }
                }
            }
    ...
    private func currentTimeLine(at date: Date, columnWidth: CGFloat, columnCount: Int) -> some View {
        let calendar = Calendar.autoupdatingCurrent
        let components = calendar.dateComponents([.hour, .minute], from: date)
        let minutesSinceMidnight = (components.hour ?? 0) * 60 + (components.minute ?? 0)
        let slotIndex = minutesSinceMidnight / grid.slotMinutes
        let yOffset = CGFloat(slotIndex) * slotHeight

        let lineWidth = timeColumnWidth + columnSpacing + CGFloat(columnCount) * (columnWidth + columnSpacing)
