...
struct AppRootView: View {
    ...
    var body: some View {
            ...
            Group {
                if let shareAcceptanceLoadingState = model.shareAcceptanceLoadingState {
                    ShareAcceptanceLoadingView(
                        ...
                        continueAction: model.continueAfterAcceptingSharedChild
                    )
        ...
        // Use safeAreaBar so the error banner respects the status bar and Dynamic Island
        // on all device families rather than overlapping them with a fixed offset.
